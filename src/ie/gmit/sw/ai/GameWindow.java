...
public class GameWindow extends Application{
	...
	private LongProperty ShowHealth = new SimpleLongProperty(0);
	@Override
    public void start(Stage stage) throws Exception {
		...
		VBox box = new VBox();
		Health.setTextFill(Color.color(0, 0.4, 0.4));
		Health.setFont(new Font("verdana", 30));
	...
	private void updateView(){ 
		...
		player.setRow(currentRow); //Get player current row
		player.setCol(currentCol); //Get player current col
		Health.textProperty().bind(Bindings.createStringBinding(() -> (" HEALTH: " + player.getHealth() + "\n"
		+" Ghosts defeated: " + player.getGhostsDefeated()) + "\n" 
		+ " Enemy Health: "+ player.getAttackingEnemyHealth()+"\n"
		+ " Enemy Status: " + player.getAttackingEnemyCommand(), ShowHealth));
		//Display  health and ghosts defeated on the screen
	}
