...
public class GameModel {
	...
	private void addGameCharacters() throws Exception {
		...
		addGameCharacter(tasks, '\u0032', '0', MAX_CHARACTERS / 5,new NeuralNetEnemy(100,NeuralNetEnemy.RandStrength())); //Enemy controlled by Neural network
	    addGameCharacter(tasks, '\u0033', '0', MAX_CHARACTERS / 5,new NeuralNetEnemy(100,NeuralNetEnemy.RandStrength())); //Enemy controlled by Neural network
		addGameCharacter(tasks, '\u0034', '0', MAX_CHARACTERS / 5,new NeuralNetEnemy(100,NeuralNetEnemy.RandStrength())); //Enemy controlled by Neural network
		addGameCharacter(tasks, '\u0035', '0', MAX_CHARACTERS / 5,new FuzzyNetEnemy(100,FuzzyNetEnemy.RandStrength())); //Enemy controlled by Fuzzy network
		addGameCharacter(tasks, '\u0036', '0', MAX_CHARACTERS / 5,new FuzzyNetEnemy(100,FuzzyNetEnemy.RandStrength())); //Enemy controlled by Fuzzy network
		tasks.forEach(exec::execute);
	}
	
	private void addGameCharacter(Collection<Task<Void>> tasks, char enemyID, char replace, int number,Enemy enemy){
		int counter = 0;
		while (counter < number){
			...
			if (model[row][col] == replace){
				 ...
				 */
				tasks.add(new CharacterTask(this, enemyID, row, col, enemy));
				counter++;
			}
		}
	}
